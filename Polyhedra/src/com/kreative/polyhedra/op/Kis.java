...
public class Kis extends PolyhedronOp {
	public static enum VertexGen {
		FACE_OFFSET {
			public Point3D createVertex(List<Point3D> sv, List<Point3D> fv, double size) {
	...
	public Polyhedron op(Polyhedron seed) {
		...
		for (Polyhedron.Face f : seed.faces) {
			if (sides.isEmpty() || sides.contains(f.vertices.size())) {
				...
				Point3D newVertex = gen.createVertex(seedVertices, faceVertices, size);
				if (newVertex != null) {
