...
public class Bicupola extends PolyhedronGen {
	...
	public Polyhedron gen() {
		...
		if (e != 0) {
			Polygon.createVertices(vertices, n*2, R, 0.5, axis, e/2);
			Polygon.createVertices(vertices, n*2, R, (egyro ? 1 : 0.5), axis, -e/2);
		...
		for (int i = 0; i < n; i++) {
			...
			if (e != 0) {
				...
				if (cgyro) {
					faces.add(Arrays.asList(p+n+n, q+n+n, m+n+n, l+n+n));
					faces.add(Arrays.asList(p+n+n, l+n+n, k+n+n));
				} else {
					faces.add(Arrays.asList(p+n+n, q+n+n, l+n+n, k+n+n));
