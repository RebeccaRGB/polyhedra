...
public class Cupola extends PolyhedronGen {
	private final int n;
	...
	public Cupola(int n, double R, double r, Axis axis, double h, Color c) {
		this(n, R, r, axis, h, c, c);
	}
	public Cupola(int n, double R, double r, Axis axis, double h, Color base, Color join) {
		this.n = n;
		this.R = R;
		this.r = r;
		this.axis = axis;
	...
	public Polyhedron gen() {
		...
		List<List<Integer>> faces = new ArrayList<List<Integer>>(n*2+2);
		List<Color> faceColors = new ArrayList<Color>(n*2+2);
		Polygon.createVertices(vertices, n, r, 0, axis, h/2);
		Polygon.createVertices(vertices, n*2, R, 0.5, axis, -h/2);
		Polygon.createFaces(faces, faceColors, n, 1, 0, false, bc);
		Polygon.createFaces(faces, faceColors, n*2, 1, n, true, bc);
		for (int i = 0; i < n; i++) {
			int j = (i + 1) % n;
			int k = (n + i + i);
			int l = (n + i + i + 1);
	...
	public static class Factory extends PolyhedronGen.Factory<Cupola> {
		public String name() { return "Cupola"; }
		
		public Cupola parse(String[] args) {
			int n = 3;
			...
			double r = spec.toRadius(size, n);
			return new Cupola(n, R, r, axis, h, ((bc != null) ? bc : c), ((jc != null) ? jc : c));
		}
