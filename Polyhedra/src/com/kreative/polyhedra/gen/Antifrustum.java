...
public class Antifrustum extends PolyhedronGen {
	...
	public Polyhedron gen() {
		...
		Polygon.createVertices(vertices, n, R, 0.5, axis, e-(h+e)/2);
		if (e != 0) Polygon.createVertices(vertices, n, R, (gyro ? 1 : 0.5), axis, -(h+e)/2);
		Polygon.createFaces(faces, faceColors, n, m, 0, false, baseColor);
		Polygon.createFaces(faces, faceColors, n, m, ((e != 0) ? (n+n) : n), true, baseColor);
		for (int i = 0; i < n; i++) {
			int j = (i + m) % n;
	...
	public static class Factory extends PolyhedronGen.Factory<Antifrustum> {
		public String name() { return "Antifrustum"; }
		
		public Antifrustum parse(String[] args) {
			int n = 3;
			int m = 1;
