...
public class Antibifrustum extends PolyhedronGen {
	...
	public Polyhedron gen() {
		...
		if (e != 0) {
			Polygon.createVertices(vertices, n, R, 0.5, axis, e/2);
			Polygon.createVertices(vertices, n, R, (gyro ? 1 : 0.5), axis, -e/2);
		...
		for (int i = 0; i < n; i++) {
			...
			if (e != 0) {
				...
				faces.add(Arrays.asList(j+n+n, i+n+n, j+n+n+n));
				faces.add(Arrays.asList(i+n+n, i+n+n+n, j+n+n+n));
				...
				faces.add(Arrays.asList(j, i, i+n));
				faces.add(Arrays.asList(j, i+n, j+n));
				faces.add(Arrays.asList(j+n, i+n, j+n+n));
				faces.add(Arrays.asList(i+n, i+n+n, j+n+n));
				faceColors.add(frustumColor);
				faceColors.add(frustumColor);
				faceColors.add(frustumColor);
				faceColors.add(frustumColor);
	...
	public static class Factory extends PolyhedronGen.Factory<Antibifrustum> {
		public String name() { return "Antibifrustum"; }
		
		public Antibifrustum parse(String[] args) {
			int n = 3;
			int m = 1;
